    ...
    <section id="reviews" className="mt-14 scroll-mt-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-8 mb-8">
        <div>
          ...
          <p className="text-xs mb-1" style={{ color: "var(--color-ink3)" }}>{reviews.length} reviews</p>
          <p className="text-[11px] flex items-center gap-1.5" style={{ color: "var(--color-ink4)" }}><ShieldCheck size={12} /> Verified purchases only</p>
        </div>
      ...
      <form onSubmit={handleSubmit} className="card-premium p-5 mb-8">
        ...
        <div className="flex items-center gap-1 mb-3">
          {[1, 2, 3, 4, 5].map((s) => (
            <button key={s} type="button" onClick={() => setRating(s)} aria-label={`Rate ${s} out of 5 stars`} aria-pressed={s === rating} className="flex items-center justify-center min-w-[28px] min-h-[28px] -m-0.5 p-0.5"><Star size={20} fill={s <= rating ? "var(--color-gold)" : "none"} style={{ color: "var(--color-gold)" }} /></button>
          ))}
        </div>
        ...
        <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Your review..." rows={3} className="w-full px-3 py-2 rounded-xl text-sm resize-none" style={{ border: "1px solid var(--color-border)", background: "var(--color-surface2)", color: "var(--color-ink)" }} />
        <button type="submit" disabled={sending} className="btn btn-accent mt-3"><Send size={14} /> {sending ? "Sending..." : "Publish"}</button>
      </form>
            ...
            <div className="flex items-start gap-3 mb-2">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {r.verified && <span className="flex items-center gap-1 text-[10px] font-bold" style={{ color: "var(--color-success)" }}><ShieldCheck size={11} /> Verified purchase</span>}
                </div>
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <span className="text-[11px]" style={{ color: "var(--color-ink4)" }}>{new Date(r.createdAt).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" })}</span>
                </div>
              </div>
            </div>
