...
/** Convertit une valeur (string numérique) pouces<->cm, 1 décimale max. */
export function convertMeasure(
  ...
  if (fromUnit === toUnit || (fromUnit === "cm" && toUnit === "cm")) return raw;
  const n = parseFloat(raw);
  if (!Number.isFinite(n)) return raw;
  if (fromUnit === "inches" && toUnit === "cm") {
    const cm = Math.round(n * 2.54 * 10) / 10;
    return String(cm);
  }
  if (fromUnit === "cm" && toUnit === "in") {
    const inch = Math.round((n / 2.54) * 10) / 10;
    return String(inch);
  }
  return raw;
}
...
export default function SizeGuideModal({ isOpen, sizeGuide, onClose }: { isOpen: boolean; sizeGuide?: any; onClose: () => void }) {
  ...
  const tableUnit: string = mainTable?.unit === "cm" ? "cm" : "inches";
  const unitLabel = unit === "cm" ? "cm" : "in";
  return (
    <div className="fixed inset-0 z-65">
      <div className="absolute inset-0 animate-fade-in" style={{ background: "rgba(15,13,10,.6)" }} onClick={onClose} />
      <div className="absolute inset-x-0 bottom-0 sm:inset-0 sm:m-auto sm:max-w-2xl w-full max-h-[85vh] overflow-y-auto rounded-t-4xl sm:rounded-4xl animate-fade-up" style={{ background: "var(--color-bg)", boxShadow: "var(--shadow-xl)" }}>
