    ...
    <div className="fixed inset-0 z-65">
      ...
      <div className="absolute inset-x-0 bottom-0 sm:inset-0 sm:m-auto sm:max-w-lg w-full max-h-[85vh] overflow-y-auto rounded-t-4xl sm:rounded-4xl animate-fade-up" style={{ background: "var(--color-bg)", boxShadow: "var(--shadow-xl)" }}>
        <div className="flex items-center justify-between px-6 h-16" style={{ borderBottom: "1px solid var(--color-border)" }}>
          <span className="text-sm font-bold flex items-center gap-2" style={{ color: "var(--color-ink)" }}><Ruler size={16} /> Guide des tailles</span>
          <button onClick={onClose} aria-label="Fermer" className="btn-icon w-9 h-9"><X size={16} /></button>
        </div>
        <div className="p-6">
          {mainTable ? (
            <>
              <div className="flex items-center justify-end gap-1 mb-4">{(["cm", "in"] as const).map((u) => <button key={u} onClick={() => setUnit(u)} className="chip" data-active={unit === u}>{u === "cm" ? "Centimètres" : "Pouces"}</button>)}</div>
              <div className="overflow-x-auto"><table className="w-full text-sm border-collapse"><thead><tr style={{ borderBottom: "1px solid var(--color-border)" }}><th className="text-left py-2.5 font-bold" style={{ color: "var(--color-ink)" }}>Taille</th>{(mainTable.measurements || []).map((m: any) => <th key={m.type_label} className="text-left py-2.5 font-bold" style={{ color: "var(--color-ink)" }}>{m.type_label}</th>)}</tr></thead><tbody>{availableSizes.map((size: string) => <tr key={size} style={{ borderBottom: "1px solid var(--color-border)" }}><td className="py-2.5 font-bold" style={{ color: "var(--color-ink)" }}>{size}</td>{(mainTable.measurements || []).map((m: any) => { const v = m.values?.find((x: any) => x.size === size); const disp = v?.min_value && v?.max_value ? `${v.min_value}-${v.max_value}` : v?.value || "—"; return <td key={m.type_label} className="py-2.5" style={{ color: "var(--color-ink2)" }}>{disp} {m.unit !== "none" ? m.unit : ""}</td>; })}</tr>)}</tbody></table></div>
            </>
          ) : (
            <p className="text-sm" style={{ color: "var(--color-ink2)" }}>Unisexe : S(48cm) M(51cm) L(54cm) XL(57cm)</p>
          )}
        </div>
      </div>
    </div>
