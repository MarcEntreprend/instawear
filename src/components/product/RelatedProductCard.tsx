    ...
    <div className="w-36 sm:w-40 shrink-0 snap-start">
      <div onClick={() => onSelect(product)} className="block w-full text-left card-premium overflow-hidden">
        <div className="relative aspect-square">
          ...
          <button type="button" onClick={(e) => { e.stopPropagation(); onQuickAdd(product); }} aria-label="Ajout rapide" className="absolute bottom-2 right-2 w-7 h-7 rounded-full flex items-center justify-center" style={{ background: "var(--color-accent)", color: "#fff" }}><Plus size={13} /></button>
        </div>
        <div className="p-2.5">
