        ...
        >
          ...
          <div>
            ...
            <p
              style={{ fontSize: 11, color: "var(--color-ink4)", marginTop: 4 }}
            >
              The range estimated is R$ 12.99–R$ 48.99 by Printful. Suggested
              value: 48.99 (maximum).
            </p>
          </div>
...
//   const [printfulCost, setPrintfulCost] = useState<number>(0); // coût d'impression réel
//   const [shippingEstimate, setShippingEstimate] = useState<number>(65.99); // frais de port estimés
//   const [marginPercent, setMarginPercent] = useState<number>(50); // % de marge
//   const [pfCurrency, setPfCurrency] = useState<string>("BRL");
//   const [costLoading, setCostLoading] = useState(false);
...
//   // Récupérer le coût d'impression quand la variante change
//   useEffect(() => {
//     if (!selectedVariantId || variants.length === 0) return;
//     const v = variants.find((v: any) => v.id.toString() === selectedVariantId);
...
//     const catalogVariantId = v.product.variant_id; // ex: 12829
//     setCostLoading(true);
//     fetch(`https://api.printful.com/products/${catalogVariantId}`)
//       .then((res) => {
//         if (!res.ok)
//           throw new Error("Impossible de récupérer le coût d'impression");
//         return res.json();
//       })
//       .then((data) => {
//         const cost = data?.result?.variant?.price;
//         if (typeof cost === "string") {
//           setPrintfulCost(parseFloat(cost));
//         } else if (typeof cost === "number") {
//           setPrintfulCost(cost);
...
//   // Recalcul du prix de vente lorsque les trois facteurs changent
//   useEffect(() => {
//     if (printfulCost > 0) {
//       const newPrice =
//         (printfulCost + shippingEstimate) * (1 + marginPercent / 100);
...
//         printfulPrice: printfulCost, // coût d'impression réel
//         printfulCurrency: pfCurrency,
//         shippingEstimate: shippingEstimate, // sauvegarde des frais de port estimés
//         ratings: { score: 5, count: 0 },
//         boughtLastMonth: 0,
//       };
...
//         {/* Image principale et galerie (inchangées) */}
//         <div>
//           <label style={labelStyle}>Image principale (URL)</label>
//           <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
//             <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
...
//         {/* Coût d'impression (récupéré automatiquement) */}
//         <div
//           style={{
//             background: "var(--color-surface2)",
//             borderRadius: 10,
...
//           )}
//           <span
//             style={{ fontSize: 11, marginLeft: 8, color: "var(--color-ink4)" }}
//           >
//             (prix payé à Printful)
//           </span>
//         </div>

//         {/* Frais de port estimés */}
//         <div>
//           <label style={labelStyle}>Frais de port estimés ({pfCurrency})</label>
//           <input
//             type="number"
//             value={shippingEstimate}
//             onChange={(e) =>
...
//           }}
//         >
//           Prix de vente calculé : {price.toFixed(2)} {pfCurrency}
//         </div>
