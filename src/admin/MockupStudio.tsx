...
// color_images, gallery + gallery_meta, product_mockups) : rien d'autre
// n'est touché. Resyncs ultérieurs préservent les visuels storage
// (cf. _shared/productImages.ts). Compteurs affichés = APPLIQUÉS
...
export default function MockupStudio({
  ...
  const handleExportJobs = () => {
    const rows = [
      "job_id;produit;statut;tentatives;task_key;erreur;mockups;appliqués;sans_correspondance;mis_a_jour",
    ];
    for (const j of status?.jobs || []) {
      const r = (j.result || {}) as any;
      const orphan: string[] = [
        ...(Array.isArray(r.unmatchedVids) ? r.unmatchedVids : []),
        ...(Array.isArray(r.unmatchedHexes) ? r.unmatchedHexes : []),
      ].map(String);
      rows.push(
        [
          j.id,
          ...
          r.mockupsGenerated ?? "",
          typeof r.applied === "number" ? r.applied : "",
          `"${orphan.slice(0, 12).join(" ").replace(/"/g, '""')}"`,
          j.updated_at,
        ].join(";"),
      );
    }
              ...
              const detail =
                j.status === "done"
                  ? (() => {
                      const r = (j.result || {}) as any;
                      const applied =
                        typeof r.applied === "number" ? r.applied : null;
                      const orphan: string[] = [
                        ...(Array.isArray(r.unmatchedVids) ? r.unmatchedVids : []),
                        ...(Array.isArray(r.unmatchedHexes) ? r.unmatchedHexes : []),
                      ].map(String);
                      const base = `${r.mockupsGenerated ?? "?"} mockup(s)`;
                      if (applied == null) return base;
                      return (
                        `${base} · ${applied} appliqué(s)` +
                        (orphan.length > 0
                          ? ` · sans correspondance : ${orphan.slice(0, 6).join(", ")}`
                          : "")
                      );
                    })()
                  : j.status === "failed"
            ...
            >
              ...
              {status.jobs
                ...
                .reduce(
                  (s, j) =>
                    s +
                    (typeof (j.result as any)?.applied === "number"
                      ? (j.result as any).applied
                      : Number((j.result as any)?.mockupsGenerated || 0)),
                  0,
                )}{" "}
              appliqué(s)
              <span
