...
export default function OrderTrackingPage({ initialCode = "", onBack }: { initialCode?: string; onBack: () => void }) {
  ...
  usePageMeta({
    ...
    description: "Track your InstaWear order in real time with your ORD number.",
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[var(--color-bg)] animate-fade-in">
      <div className="max-w-175 mx-auto px-4 sm:px-6 pt-5 pb-2 flex items-center gap-2">
        ...
        <span className="text-xs" style={{ color: "var(--color-ink3)" }}>InstaWear / Order tracking</span>
      </div>
      <div className="max-w-175 mx-auto px-4 sm:px-6 py-10">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-extrabold mb-2" style={{ color: "var(--color-ink)" }}>Track my order</h1>
          <p className="text-sm" style={{ color: "var(--color-ink3)" }}>Enter the order number received by email, format ORD-2026-XXXXXX.</p>
        </div>
        <form onSubmit={handleSubmit} className="flex items-center gap-2 mb-10 max-w-md mx-auto">
          ...
          <button type="submit" disabled={loading} className="btn btn-accent shrink-0">{loading ? "..." : "Track"}</button>
        </form>
        {result === "not-found" && <div className="text-center py-8"><p className="text-sm font-semibold" style={{ color: "var(--color-negative)" }}>No order matches this number.</p></div>}
          ...
          <div className="card-premium p-6 animate-fade-up max-w-md mx-auto">
            <div className="flex items-center justify-between mb-6 flex-wrap gap-2">
              <div>
                ...
                <p className="text-xs" style={{ color: "var(--color-ink3)" }}>Ordered on {result.created_at ? new Date(result.created_at).toLocaleDateString("en-US") : ""}</p>
              </div>
            </div>
            ...
            {result.tracking_info?.[0]?.tracking_number && (
              <div className="flex items-center justify-between gap-3 p-3.5 rounded-xl mb-6" style={{ background: "var(--color-surface2)" }}>
                ...
                {result.tracking_info[0].tracking_url && <a href={result.tracking_info[0].tracking_url} target="_blank" rel="noreferrer" className="text-xs font-bold flex items-center gap-1 shrink-0" style={{ color: "var(--color-accent)" }}>Track <ExternalLink size={11} /></a>}
              </div>
            )}
          </div>
        )}
