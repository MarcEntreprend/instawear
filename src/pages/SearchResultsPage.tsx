...
export default function SearchResultsPage({
  ...
  usePageMeta({
    ...
    description:
      "Search for t-shirts, hoodies and InstaWear accessories for your next event.",
      ...
      <div className="max-w-350 mx-auto px-4 sm:px-6 pt-5 pb-6">
        <div className="flex items-center gap-2 mb-5">
          ...
          <form
            ...
            className="flex items-center flex-1 max-w-xl rounded-full px-4 h-11"
            style={{
        ...
        {query.trim() && (
          <p className="text-sm mb-6" style={{ color: "var(--color-ink3)" }}>
        ...
        {baseResults.length > 0 && (
          <div className="flex items-center gap-2 overflow-x-auto no-scrollbar mb-8">
            <button
              ...
              className="chip"
              data-active={activeEventType === null}
            >
              All
            </button>
            {EVENT_TYPES.filter((et) =>
              baseResults.some((p) => p.eventType === et.value),
            ).map(({ value, label, icon: Icon }) => (
              <button
                key={value}
                onClick={() => setActiveEventType(value)}
                className="chip"
                data-active={activeEventType === value}
              >
                <Icon size={14} /> {label}
              </button>
            ))}
        ...
        {results.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
            {results.map((product) => (
              <StoreProductCard
                ...
                onAddToCart={(p, c, s) => onQuickAdd(p)}
          ...
          <div className="text-center py-16">
            ...
            >
              <SearchX size={24} style={{ color: "var(--color-ink4)" }} />
            </span>
            <p
            ...
            <p className="text-sm mb-8" style={{ color: "var(--color-ink3)" }}>
              {query.trim()
                ? "Try another keyword or explore these suggestions."
