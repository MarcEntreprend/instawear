  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-(--color-bg) animate-fade-in">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-4 pb-16">
        ...
        <div
          className="bg-white border rounded-2xl overflow-hidden shadow-xl"
          ...
          <div className="flex flex-col lg:flex-row gap-8 p-6 md:p-8">
                ...
                <div className="flex sm:flex-col gap-2 order-2 sm:order-first sm:w-16 shrink-0">
                  {variantImages.map((v) => (
                    <button
                      ...
                      className={`w-12 h-12 sm:w-14 sm:h-14 rounded-lg overflow-hidden border-2 ${pickedColor === v.color ? "border-(--color-accent)" : "border-gray-200"}`}
                    >
                      <img
                        src={v.image}
                ...
                <div className="flex sm:flex-col gap-2 order-2 sm:order-first sm:w-16 shrink-0">
                  {allImages.map((img, idx) => (
                    <button
                      ...
                      className={`w-12 h-12 sm:w-14 sm:h-14 rounded-lg overflow-hidden border-2 ${activeGalleryIndex === idx ? "border-(--color-accent)" : "border-gray-200"}`}
                    >
                      <img
                        src={img}
                        alt=""
              ...
              <div className="w-full aspect-3/4">
                <ImageZoom src={displayImage} alt={product.title}>
                  ...
                  {product.isLimitedTime && (!dealExpired || dealFadingOut) && (
                    <span
                      className={`absolute top-3 right-3 bg-rose-500 text-white text-[10px] font-black uppercase px-2.5 py-1 rounded-full shadow z-10 ${dealFadingOut ? "deal-fade-out" : "animate-pulse"}`}
                    >
                      LIMITED
                    </span>
                  )}
            ...
            <div className="lg:w-[25%]">
                  ...
                  {product.showRatings && (
                    <div className="flex items-center text-amber-400">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-3.5 h-3.5 ${i < Math.floor(product.ratings.score) ? "fill-amber-400 text-amber-400" : "text-gray-300"}`}
                        />
                      ))}
                ...
                <div className="flex flex-wrap gap-2">
                  {dispColors.map((c, idx) => {
                    ...
                    return (
                      <button
                        key={idx}
                        onClick={() => !blocked && setPickedColor(c)}
                        disabled={blocked}
                        className={`w-9 h-9 rounded-full border-2 p-0.5 ${isPicked ? "border-cyan-400 scale-105 shadow-md" : "border-gray-200"} ${blocked ? "opacity-40 cursor-not-allowed grayscale" : ""}`}
              ...
              <div className="mt-4">
                ...
                {sizeGuideOpen && (
                  <div className="p-3 bg-gray-50 border rounded-lg text-[10px] text-gray-500 mb-3 max-h-60 overflow-y-auto">
                    {product.sizeGuide &&
                    (product.sizeGuide.size_tables ||
                      product.sizeGuide.measurements) ? (
                      <SizeGuideDisplay sizeGuide={product.sizeGuide} />
                    ) : (
                      <p>Unisex fit: S(48cm) M(51cm) L(54cm) XL(57cm)</p>
                    )}
                  </div>
                )}
                  ...
                  {dispSizes.map((s) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={blocked}
                        className={`min-w-10 h-8 rounded border text-xs font-bold uppercase px-2.5 ${pickedSize === s ? "border-cyan-400 bg-(--color-accent-bg)" : "border-gray-200 bg-gray-50/60"} ${blocked ? "opacity-40 line-through cursor-not-allowed" : ""}`}
                      >
                        {s}
                      </button>
                    );
              ...
              <div
                className="mt-5 text-xs text-gray-600 leading-relaxed border-b pb-5"
                style={{ borderColor: "var(--color-border)" }}
              >
              ...
              <div className="bezel-outer">
                <div className="bezel-inner p-5 flex flex-col gap-4">
                  {product.dealActive && !dealExpired && product.dealPrice ? (
                    <div>
                      ...
                      <p className="text-xs font-semibold text-rose-500">
                        ...
                        % Limited deal{" "}
                        {countdownString && (
                          <span className="font-mono">({countdownString})</span>
                        )}
                      </p>
                    </div>
                  ) : (
                    ...
                    if (blocked) {
                      ...
                      return (
                        <div className="text-center">
                          <p className="text-xs text-rose-500 mb-3">{msg}</p>
                          <button
                            disabled
                            className="w-full bg-gray-200 text-gray-400 font-black text-xs py-3.5 rounded-xl uppercase cursor-not-allowed"
                          >
                            Add to cart
                          </button>
                        </div>
                    ...
                    return (
                      <div className="flex flex-col gap-2">
                        ...
                        <button
                          ...
                          style={{ background: "#f59e0b", color: "#1a1916" }}
                        >
                          Buy now
                        </button>
                        <button
                  ...
                  >
                    ...
                    >
                      <Truck size={14} /> FREE on orders over $35
                    </p>
                    <p>
