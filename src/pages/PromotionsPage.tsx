...
export default function PromotionsPage({ products, favorites, dealExpired, dealFadingOut, countdownString, currencySymbol, onToggleFavorite, onAddToCart, onSelectProduct, onBack }: {
  ...
  usePageMeta({
    ...
    description: "InstaWear print-on-demand deals: t-shirts, hoodies and event accessories on sale.",
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[var(--color-bg)] animate-fade-in">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10">
        ...
        <h1 className="text-2xl sm:text-3xl font-extrabold mt-2 mb-2" style={{ color: "var(--color-ink)" }}>Current promotions</h1>
        <p className="text-sm mb-8" style={{ color: "var(--color-ink3)" }}>{deals.length} items on sale</p>
        {deals.length === 0 ? (
          <div className="card-premium p-8 text-center"><p className="text-sm" style={{ color: "var(--color-ink3)" }}>No active promotions at the moment.</p></div>
        ) : (
