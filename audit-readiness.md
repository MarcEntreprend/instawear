...
- SEO : `index.html` (robots, OG absolus, `Organization` + `WebSite+SearchAction`), `usePageMeta` 7 pages, JSON-LD `Product`/`FAQPage`/`BreadcrumbList`, `robots.txt`/`llms.txt`/`ai.txt`/`humans.txt`, sitemap auto (`prebuild`), `llms.txt` bloc produits auto, `site.webmanifest` + link, **prerender statique** (`scripts/prerender.ts` via `postbuild` + `vercel.json` `cleanUrls`, snapshots en `<noscript>` = zéro flash, SEO intact) — plus aucun risque SPA.
- LCP hero : `fetchPriority high` + précharge runtime (`<link rel=preload>`) dès `heroBanners` connu ; storage Supabase sert les images en `no-cache` malgré métadonnées 3600 (constaté headers) → la solution cache est le CDN ImageKit, pas un param d'upload.
