// supabase/functions/email-preferences/_shared/rateLimit.ts
// Copie locale (le bundler n'inclut que le dossier de la fonction).
// Miroir de supabase/functions/_shared/rateLimit.ts + quota email-preferences.
...
export function quotaFor(path: string): Quota {
  switch (path) {
    case "email-preferences": return { max: 10, windowMs: 60_000 };
