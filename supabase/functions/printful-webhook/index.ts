    ...
    try {
      ...
      if (event.type === "checkout.session.completed") {
        ...
        if (orderId) {
          ...
          if (order) {
            // ── Vérification du montant payé vs total de la commande ──
            // Le montant est recalculé côté serveur lors de la création de la
            // session (stripe-checkout). On refuse de valider si le montant
            // réellement payé ne correspond pas.
            const expectedTotalCents = Math.round(
              Number(order.total_amount) * 100,
            );
            const paidTotalCents = session.amount_total ?? 0;

            if (paidTotalCents !== expectedTotalCents) {
              console.error(
                `[stripe-webhook] Mismatch montant commande ${orderId}: payé=${paidTotalCents} (${session.currency}), attendu=${expectedTotalCents}. Commande NON validée.`,
              );
              return new Response(
                JSON.stringify({
                  received: true,
